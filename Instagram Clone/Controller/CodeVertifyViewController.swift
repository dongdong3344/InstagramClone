...
class CodeVertifyViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setupAttributedTagLabel()
        
        setupTimerCountDown()
    
    }
    func setupAttributedTagLabel(){
        
        let attributedStr = NSMutableAttributedString(string: "请输入我们发送到\(phoneNumber!)的验证码。")
        
        let range:NSRange? = NSMakeRange(attributedStr.length - phoneNumber.characters.count - 5, phoneNumber.characters.count)
