...
class RegisterViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let codeVertifyVC = segue.destination as? CodeVertifyViewController, let phone = phoneNumberTextField.text,let countryCode = codeButton.currentTitle?.components(separatedBy: " ").last else {
            return
        }
        
        let phoneNumber = countryCode + phone
        codeVertifyVC.phoneNumber = phoneNumber
    }
